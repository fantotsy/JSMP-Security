...
class MainPage(webapp.RequestHandler):
  ...
  def mapHtmlSpecialChars(self, query):
    result = query.replace("&", "&amp;")
    result = result.replace("<", "&lt;")
    return result.replace(">", "&rt;")
  ...
  def get(self):
    ...
    if not self.request.get('query'):
      ...
      mappedQuery = self.mapHtmlSpecialChars(query)
      message = "Sorry, no results were found for <b>" + mappedQuery + "</b>."
